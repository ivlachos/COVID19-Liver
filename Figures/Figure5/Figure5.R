...
############# UMAP plots
seur_obj_immune <- seur_obj[,seur_obj$Major_compartments == "Immune"]

umap_1 <- as.data.frame(umap_embeddings$`1`)

plot_df <- cbind.data.frame(umap_1, seur_obj_immune$Label); 
...
seur_obj_endothelial <- seur_obj[,seur_obj$Major_compartments == "Endothelial"]

umap_1 <- as.data.frame(umap_embeddings$`2`)

plot_df <- cbind.data.frame(umap_1, seur_obj_endothelial$Label); 
...
seur_obj_mesenchymal <- seur_obj[,seur_obj$Major_compartments == "Stellate"]

umap_1 <- as.data.frame(umap_embeddings$`3`)

plot_df <- cbind.data.frame(umap_1, seur_obj_mesenchymal$Label); 
...
figureE <- ggplot(plot_df, aes(x = UMAP1, y = UMAP2)) +
  geom_point(data = plot_df, pch = 21, color = 'black', size = 0.05,alpha = 0.4) +
  geom_point(aes(color = SubCluster), alpha = 0.4, size = 0.05, show.legend = T) +
  ggrepel::geom_text_repel(data = plot_df2,aes(label = SubCluster), size=2.7, max.iter = 60) +
  scale_color_manual(values = cols, 
                     labels = paste0(levels(factor(plot_df$SubCluster)))) +
